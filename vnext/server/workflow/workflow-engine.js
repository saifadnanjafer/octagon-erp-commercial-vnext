...
function mountWorkflow(deps = {}) {
  ...
  function normalizeWorkflow(input, existing = {}) {
    ...
    return {
      ...
      version: Number(existing.version || 1) + (input.name !== undefined || input.nodes !== undefined ? 1 : 0),
      trigger: {
        type,
  ...
  function log(run, node, status, detail) {
    // Defensive init: `run` here is not always a fresh makeRun() object — it
    // can be a run rehydrated straight from x_records JSON (e.g. by
    // recoverRunningWorkflows() after a restart, or a legacy/imported row),
    // which is an external-boundary read and must not be assumed to carry
    // every field a freshly-created run has. Without this guard, any such
    // rehydrated run without a `logs` array crashes every subsequent log()
    // call with "Cannot read properties of undefined (reading 'push')".
  ...
  async function executeNodeWithRetry(node, context, run) {
    const config = interpolateDeep(node.config || {}, context);
    if (node.type === 'webhook') {
      const maxRetries = Math.min(5, Math.max(0, Number(config.max_retries || 0)));
      let delay = Math.max(100, Number(config.backoff_ms || 1000));
      let lastErr = null;
      for (let attempt = 0; attempt <= maxRetries; attempt++) {
        try {
          return await executeNode(node, context, run);
        } catch (err) {
          lastErr = err;
    ...
    return executeNode(node, context, run);
  }

  async function runWorkflow(workflow, trigger, context) {
    if (!workflow || workflow.active === false) return null;

    // Rate limiting (max 60 runs per minute per workflow)
    const oneMinAgo = new Date(Date.now() - 60000).toISOString();
    const countRow = db.prepare("SELECT COUNT(*) AS n FROM x_records WHERE entity = ? AND removed = 0 AND created_at > ? AND json_extract(data, '$.workflow_id') = ?")
      .get('workflow_run', oneMinAgo, workflow.id);
    if (Number(countRow?.n || 0) >= 60) {
      console.warn(`[workflow-engine] Rate limit exceeded for workflow ${workflow.id}`);
      return null;
    }

    // Loop detection (max depth 10)
    if (context.depth && context.depth > 10) {
      console.error(`[workflow-engine] Loop detected for workflow ${workflow.id} (depth = ${context.depth})`);
      return null;
    }

    const nextDepth = (context.depth || 0) + 1;
    const runContext = { ...context, depth: nextDepth };

    const run = makeRun(workflow, trigger, runContext);
    try {
      for (let i = 0; i < (workflow.nodes || []).length; i++) {
        ...
        const result = await executeNodeWithRetry(node, runContext, run);
  ...
  async function resumeWorkflow(workflow, run) {
    const runContext = run.context || {};
    try {
      const startIndex = run.current_node_index || 0;
  ...
  async function recoverRunningWorkflows() {
    try {
      const rows = db.prepare("SELECT id, data FROM x_records WHERE entity = ? AND removed = 0").all('workflow_run');
      for (const row of rows) {
        let run = parse(row.data, null);
        if (run && run.status === 'running') {
          console.log(`[workflow-engine] recovering run ${run.id} for workflow ${run.workflow_id}`);
          const wf = getRecord('workflow', run.workflow_id);
          if (wf) {
            // Awaited (not fire-and-forget): recoverRunningWorkflows() is
            // exported on the returned engine specifically so callers (tests,
            // orderly-shutdown code) can `await` full recovery instead of
            // racing its internal async completion. The boot-time call site
            // below still schedules this via setTimeout without awaiting it,
            // which is intentional — nothing at boot needs to block on it.
            await resumeWorkflow(wf, run);
          } else {
            run.status = 'failed';
            run.error = 'Workflow definition not found during recovery';
            run.finished_at = now();
            saveRun(run);
          }
        }
      ...
      console.error('[workflow-engine] recovery failed:', e.message);
    }
  }
  ...
  function fireRecordEvent(entity, action, record) {
    ...
    if (!event) return;
    if (deps.events && typeof deps.events.publish === 'function') {
      try {
        deps.events.publish({
          type: 'workflow.record_event', companyId: record?.company_id || record?.companyId || null,
          entity, recordId: record?.id || null, requiredPermission: 'platform:workflow:read',
          payload: { action: event, entity, recordId: record?.id || null }, createdBy: 'workflow-engine',
        });
      } catch (_) {}
    }
  ...
  // Run recovery asynchronously on startup
  setTimeout(() => recoverRunningWorkflows(), 1000);
  ...
  return { handle, runWorkflow, runDueSchedules, fireRecordEvent, recoverRunningWorkflows, stop() { clearInterval(poll); } };
}

module.exports = { mountWorkflow, isFrozen, interpolate, compare };
