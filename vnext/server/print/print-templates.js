// clean-room; behavior modeled on platform/server/print-templates.js (proprietary self, not copied)
'use strict';
...
function createPrintTemplateHandler(deps) {
  ...
  function ensureTemplatesSeeded() {
    if (!templatesSeeded) {
      try {
        ...
        // Retried on demand if table hasn't migrated yet
      }
    }
  }
  ...
  return {
    ...
    list: (entity) => {
      ensureTemplatesSeeded();
      return listTemplates(db, entity);
    },
    get: (id) => {
      ensureTemplatesSeeded();
