// R6.2 thin HTTP adapter for subscriptions and dunning.
'use strict';

const sub = require('./subscription-engine');

const API_BASE = '/api/x/subscriptions';
function envelope(data, error = null, meta = null) { return { success: !error, data: error ? null : data, error, meta }; }

function mountSubscriptionRoutes(deps = {}) {
  const { db, sendJson, readRequestBody, requireSession, resolveScope, canPermission, events } = deps;
  if (!db) throw new Error('mountSubscriptionRoutes requires db');
  const write = sendJson || ((res, status, body) => { res.writeHead(status, { 'Content-Type': 'application/json; charset=utf-8' }); res.end(JSON.stringify(body)); });
  const read = readRequestBody || (req => new Promise((resolve, reject) => { let raw = ''; req.on('data', c => { raw += c; if (raw.length > 512 * 1024) req.destroy(); }); req.on('end', () => resolve(raw)); req.on('error', reject); }));
  const inert = { setHeader() {}, writeHead() {}, end() {}, headersSent: false, writableEnded: false };

  function auth(req, res, permission) {
    const session = typeof requireSession === 'function' ? requireSession(req, inert, { allowLocalDev: false }) : null;
    if (!session || !session.ok) { write(res, 401, envelope(null, 'Login session required', { code: 'AUTH_REQUIRED' })); return null; }
    const user = { id: String(session.userId || session.user?.id || ''), userId: String(session.userId || session.user?.id || ''), groups: session.groups || [], role: session.user?.role || '', local: /^local-/.test(session.mode || '') };
    const allowed = user.local || user.groups.includes('system.admin') || user.groups.includes('admin') || (typeof canPermission === 'function' && canPermission(user, permission));
    if (!allowed) { write(res, 403, envelope(null, `Permission required: ${permission}`, { code: 'FORBIDDEN' })); return null; }
    return user;
  }

  function company(req, res, user) {
    const resolved = typeof resolveScope === 'function' ? resolveScope(req, { userId: user.id, groups: user.groups, user: { id: user.id } }) : { companyId: req.headers['x-company-id'] };
    if (resolved?.error) { write(res, resolved.error.status || 403, envelope(null, resolved.error.message, { code: resolved.error.code })); return null; }
    if (!resolved?.companyId) { write(res, 400, envelope(null, 'x-company-id is required', { code: 'COMPANY_SCOPE_REQUIRED' })); return null; }
    return { companyId: String(resolved.companyId), tenantId: String(resolved.tenantId || resolved.companyId) };
  }

  async function body(req) { const raw = await read(req); try { return raw ? JSON.parse(raw) : {}; } catch (_) { const error = new Error('Invalid JSON body'); error.statusCode = 400; error.code = 'INVALID_JSON'; throw error; } }
  function routeError(res, error) { write(res, error.statusCode || 400, envelope(null, error.message || 'Subscription request failed', { code: error.code || 'SUBSCRIPTION_ERROR' })); }

  function handle(req, res, url) {
    if (!url.pathname.startsWith(`${API_BASE}/`) && url.pathname !== API_BASE) return false;
    const rest = url.pathname.slice(API_BASE.length).split('/').filter(Boolean).map(decodeURIComponent);
    
    // Determine permissions required
    let requiredPerm = 'subscription:view';
    if (req.method === 'POST') {
      requiredPerm = 'subscription:manage';
    }
    
    const user = auth(req, res, requiredPerm);
    if (!user) return true;
    const scope = company(req, res, user); if (!scope) return true;
    const context = { ...scope, userId: user.id };

    try {
      if (rest.length === 1 && rest[0] === 'plans') {
        if (req.method === 'GET') {
          write(res, 200, envelope(sub.listPlans(db, scope.companyId)));
          return true;
        }
        if (req.method === 'POST') {
          body(req).then(input => write(res, 201, envelope(sub.createPlan(db, scope.companyId, input, user.id)))).catch(error => routeError(res, error));
          return true;
        }
      }
      
      if (rest.length === 2 && rest[0] === 'plans' && req.method === 'GET') {
        write(res, 200, envelope(sub.getPlan(db, scope.companyId, rest[1])));
        return true;
      }
      
      if (rest.length === 0) {
        if (req.method === 'GET') {
          write(res, 200, envelope(sub.listSubscriptions(db, scope.companyId)));
          return true;
        }
        if (req.method === 'POST') {
          body(req).then(input => write(res, 201, envelope(sub.createSubscription(db, scope.companyId, input, user.id)))).catch(error => routeError(res, error));
          return true;
        }
      }
      
      if (rest.length === 1 && rest[0] === 'kpis' && req.method === 'GET') {
        const asOf = url.searchParams.get('as_of') || undefined;
        write(res, 200, envelope(sub.getSubscriptionKPIs(db, scope.companyId, asOf)));
        return true;
      }
      
      if (rest.length === 1 && rest[0] === 'billing' && req.method === 'POST') {
        body(req).then(input => {
          const asOf = input.as_of || undefined;
          write(res, 200, envelope(sub.runBilling(db, scope.companyId, asOf, user.id)));
        }).catch(error => routeError(res, error));
        return true;
      }

      if (rest.length === 1 && rest[0] === 'dunning' && req.method === 'POST') {
        body(req).then(input => {
          const asOf = input.as_of || undefined;
          const options = { chargeInterest: input.charge_interest, interestRate: input.interest_rate };
          write(res, 200, envelope(sub.runDunning(db, scope.companyId, asOf, options, user.id)));
        }).catch(error => routeError(res, error));
        return true;
      }
      
      if (rest.length === 1) {
        if (req.method === 'GET') {
          write(res, 200, envelope(sub.getSubscription(db, scope.companyId, rest[0])));
          return true;
        }
      }
      
      if (rest.length === 2 && rest[0] !== 'plans' && req.method === 'POST') {
        const subId = rest[0];
        const action = rest[1];
        
        if (action === 'cancel') {
          body(req).then(input => {
            const date = input.date || undefined;
            write(res, 200, envelope(sub.cancelSubscription(db, scope.companyId, subId, date, user.id)));
          }).catch(error => routeError(res, error));
          return true;
        }
        
        if (action === 'change') {
          body(req).then(input => {
            const toPlanId = input.to_plan_id;
            const date = input.effective_date || undefined;
            write(res, 200, envelope(sub.changeSubscription(db, scope.companyId, subId, toPlanId, date, user.id)));
          }).catch(error => routeError(res, error));
          return true;
        }
      }

      write(res, 404, envelope(null, 'Subscription endpoint not found', { code: 'SUBSCRIPTION_NOT_FOUND' }));
      return true;
    } catch (error) { routeError(res, error); return true; }
  }
  return { handle };
}

module.exports = { mountSubscriptionRoutes };
