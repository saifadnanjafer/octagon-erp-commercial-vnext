// R9.3 Retail/POS pack HTTP routes.
'use strict';
...
function mountRetailRoutes(deps = {}) {
  ...
  function auth(req, res) { const s = typeof requireSession === 'function' ? requireSession(req, inert, { allowLocalDev: false }) : null; if (!s?.ok) { write(res, 401, envelope(null, 'Login session required', { code: 'AUTH_REQUIRED' })); return null; } const u = { id: String(s.userId || s.user?.id || ''), groups: s.groups || [], local: /^local-/.test(s.mode || '') }; if (!(u.local || u.groups.includes('admin') || u.groups.includes('system.admin') || (typeof canPermission === 'function' && canPermission(u, 'retail:manage')))) { write(res, 403, envelope(null, 'Permission required: retail:manage', { code: 'FORBIDDEN' })); return null; } return u; }
  function scope(req, res, user) { const s = typeof resolveScope === 'function' ? resolveScope(req, user) : { companyId: req.headers['x-company-id'] }; if (!s?.companyId) { write(res, 400, envelope(null, 'x-company-id is required', { code: 'COMPANY_SCOPE_REQUIRED' })); return null; } return String(s.companyId); }
  ...
  function handle(req, res, url) {
    ...
    const user = auth(req, res); if (!user) return true; const companyId = scope(req, res, user); if (!companyId) return true;
