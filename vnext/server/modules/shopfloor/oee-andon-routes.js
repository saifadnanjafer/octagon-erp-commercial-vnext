...
function mountOeeAndonRoutes(deps = {}) {
  const { db, sendJson, readRequestBody, requireSession, resolveScope, canPermission } = deps;
  if (!db) throw new Error('mountOeeAndonRoutes requires db');
  ...
  function routeError(res, error) { write(res, error.statusCode || 400, envelope(null, error.message || 'OEE/Andon request failed', { code: error.code || 'OEE_ANDON_ERROR' })); }
  ...
  function handle(req, res, url) {
    ...
    try {
      // GET OEE
      if (rest.length === 1 && rest[0] === 'oee' && req.method === 'GET') {
        const wc = url.searchParams.get('work_center_id');
        const plannedTime = Number(url.searchParams.get('planned_time') || 28800);
        write(res, 200, envelope(oee.computeOee(db, scope.companyId, wc, plannedTime)));
        return true;
      }
      
      // Andon routes
      if (rest.length === 2 && rest[0] === 'andon' && rest[1] === 'raise' && req.method === 'POST') {
        body(req).then(input => {
          write(res, 200, envelope(oee.raiseAndonCall(db, scope.companyId, input.work_order_id, input.operator_id, input.reason, input.description)));
        }).catch(error => routeError(res, error));
        return true;
      }
      
      if (rest.length === 3 && rest[0] === 'andon') {
        const callId = rest[1];
        const action = rest[2];
        
        if (action === 'acknowledge' && req.method === 'POST') {
          body(req).then(input => {
            write(res, 200, envelope(oee.acknowledgeAndonCall(db, scope.companyId, callId, input.operator_id)));
          }).catch(error => routeError(res, error));
          return true;
        }
        ...
        if (action === 'resolve' && req.method === 'POST') {
          body(req).then(input => {
            write(res, 200, envelope(oee.resolveAndonCall(db, scope.companyId, callId, user.id)));
          }).catch(error => routeError(res, error));
          return true;
        }
      }
      ...
      // Downtime routes
      if (rest.length === 2 && rest[0] === 'downtime' && rest[1] === 'start' && req.method === 'POST') {
        body(req).then(input => {
          write(res, 200, envelope(oee.logDowntimeStart(db, scope.companyId, input.work_center_id, input.reason, input.type)));
        }).catch(error => routeError(res, error));
        return true;
      }
      
      if (rest.length === 3 && rest[0] === 'downtime') {
        const dtId = rest[1];
        const action = rest[2];
        
        if (action === 'end' && req.method === 'POST') {
          write(res, 200, envelope(oee.logDowntimeEnd(db, scope.companyId, dtId)));
          return true;
        }
      }
...
module.exports = { mountOeeAndonRoutes };
