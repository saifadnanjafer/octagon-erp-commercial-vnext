...
function mountQualityRoutes(deps = {}) {
  ...
  function routeError(res, error) { write(res, error.statusCode || 400, envelope(null, error.message || 'Quality request failed', { code: error.code || 'QUALITY_ERROR' })); }
  ...
  function handle(req, res, url) {
    ...
    try {
      if (rest.length === 1 && rest[0] === 'templates' && req.method === 'POST') {
        body(req).then(input => {
          write(res, 201, envelope(qual.createQualityTemplate(db, scope.companyId, input)));
        }).catch(error => routeError(res, error));
        return true;
      }
      
      if (rest.length === 1 && rest[0] === 'inspections' && req.method === 'POST') {
        body(req).then(input => {
          write(res, 201, envelope(qual.createInspection(db, scope.companyId, input)));
        }).catch(error => routeError(res, error));
        return true;
      }
      
      if (rest.length === 3 && rest[0] === 'inspections') {
        const inspId = rest[1];
        const action = rest[2];
        if (action === 'record' && req.method === 'POST') {
          body(req).then(input => {
            write(res, 200, envelope(qual.recordInspectionResult(db, scope.companyId, inspId, input.status, input.measured_values, user.id)));
          }).catch(error => routeError(res, error));
          return true;
        }
      }
      
      if (rest.length === 3 && rest[0] === 'ncr') {
        const ncrId = rest[1];
        const action = rest[2];
        if (action === 'workflow' && req.method === 'POST') {
          body(req).then(input => {
            write(res, 200, envelope(qual.updateNcrWorkflow(db, scope.companyId, ncrId, input.state, input)));
          }).catch(error => routeError(res, error));
          return true;
        }
      }
      
      if (rest.length === 2 && rest[0] === 'ncr' && rest[1] === 'pareto' && req.method === 'GET') {
        write(res, 200, envelope(qual.getDefectParetoReport(db, scope.companyId)));
        return true;
      }
...
module.exports = { mountQualityRoutes };
