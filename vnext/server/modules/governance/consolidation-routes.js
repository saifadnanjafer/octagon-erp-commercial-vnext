...
function mountConsolidationRoutes(deps = {}) {
  const { db, sendJson, readRequestBody, requireSession, resolveScope, canPermission } = deps;
  if (!db) throw new Error('mountConsolidationRoutes requires db');
  ...
  function routeError(res, error) { write(res, error.statusCode || 400, envelope(null, error.message || 'Governance request failed', { code: error.code || 'GOVERNANCE_ERROR' })); }
  ...
  function handle(req, res, url) {
    ...
    try {
      if (rest.length === 2 && rest[0] === 'intercompany' && rest[1] === 'rules' && req.method === 'POST') {
        body(req).then(input => {
          write(res, 201, envelope(cons.createIntercompanyRule(db, scope.companyId, input)));
        }).catch(error => routeError(res, error));
        return true;
      }
      
      if (rest.length === 4 && rest[0] === 'intercompany' && rest[1] === 'rules' && rest[3] === 'mirror' && req.method === 'POST') {
        const docId = rest[2];
        body(req).then(input => {
          write(res, 200, envelope(cons.triggerIntercompanyMirror(db, scope.companyId, input.document_type, docId, user.id)));
        }).catch(error => routeError(res, error));
        return true;
      }
      
      if (rest.length === 2 && rest[0] === 'consolidation' && rest[1] === 'rates' && req.method === 'POST') {
        body(req).then(input => {
          write(res, 201, envelope(cons.createConsolidationRate(db, scope.companyId, input)));
        }).catch(error => routeError(res, error));
        return true;
      }
      
      if (rest.length === 2 && rest[0] === 'consolidation' && rest[1] === 'report' && req.method === 'GET') {
        const sourceCompanies = url.searchParams.get('source_company_ids') ? url.searchParams.get('source_company_ids').split(',') : [];
        const reportType = url.searchParams.get('report_type') || 'trial_balance';
        write(res, 200, envelope(cons.getConsolidatedReport(db, scope.companyId, sourceCompanies, reportType)));
        return true;
      }
...
module.exports = { mountConsolidationRoutes };
