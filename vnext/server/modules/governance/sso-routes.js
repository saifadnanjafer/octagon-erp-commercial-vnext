...
function mountSsoRoutes(deps = {}) {
  ...
  function routeError(res, error) { write(res, error.statusCode || 400, envelope(null, error.message || 'SSO/Security policy request failed', { code: error.code || 'SSO_ERROR' })); }
  ...
  function handle(req, res, url) {
    ...
    try {
      if (rest.length === 2 && rest[0] === 'sso' && rest[1] === 'config' && req.method === 'POST') {
        body(req).then(input => {
          write(res, 201, envelope(sso.createSsoConfig(db, scope.companyId, input)));
        }).catch(error => routeError(res, error));
        return true;
      }
      
      if (rest.length === 2 && rest[0] === 'sso' && rest[1] === 'link' && req.method === 'POST') {
        body(req).then(input => {
          write(res, 201, envelope(sso.linkUserSso(db, user.id, input.provider_type, input.sso_uid)));
        }).catch(error => routeError(res, error));
        return true;
      }
      
      if (rest.length === 2 && rest[0] === 'sso' && rest[1] === 'authenticate' && req.method === 'POST') {
        body(req).then(input => {
          write(res, 200, envelope(sso.authenticateSsoUser(db, scope.companyId, input.provider_type, input.sso_uid, input.email, input.name)));
        }).catch(error => routeError(res, error));
        return true;
      }
      
      if (rest.length === 2 && rest[0] === 'security' && rest[1] === 'policy' && req.method === 'POST') {
        body(req).then(input => {
          write(res, 200, envelope(sso.configureSecurityPolicy(db, scope.companyId, input)));
        }).catch(error => routeError(res, error));
        return true;
      }
      
      if (rest.length === 3 && rest[0] === 'security' && rest[1] === 'policy' && rest[2] === 'mfa' && req.method === 'GET') {
        write(res, 200, envelope(sso.verifyMfaEnforcement(db, scope.companyId, user.id)));
        return true;
      }
