...
function mountSupportRoutes(deps = {}) {
  const { db, sendJson, readRequestBody, requireSession, resolveScope, canPermission } = deps;
  if (!db) throw new Error('mountSupportRoutes requires db');
  ...
  function routeError(res, error) { write(res, error.statusCode || 400, envelope(null, error.message || 'Support request failed', { code: error.code || 'SUPPORT_ERROR' })); }
  ...
  function handle(req, res, url) {
    ...
    try {
      if (rest.length === 3 && rest[0] === 'upgrade' && rest[1] === 'log' && req.method === 'POST') {
        body(req).then(input => {
          write(res, 201, envelope(support.recordUpgradeHistory(db, input.version, input.status, input.log_output, user.id)));
        }).catch(error => routeError(res, error));
        return true;
      }
      
      if (rest.length === 1 && rest[0] === 'diagnostics' && req.method === 'GET') {
        write(res, 200, envelope(support.getSystemDiagnostics(db, scope.companyId)));
        return true;
      }
      
      if (rest.length === 3 && rest[0] === 'support' && rest[1] === 'bundle' && req.method === 'GET') {
        write(res, 200, envelope(support.exportSupportBundle(db, scope.companyId)));
        return true;
      }
