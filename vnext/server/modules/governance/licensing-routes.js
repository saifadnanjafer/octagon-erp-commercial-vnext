...
function mountLicensingRoutes(deps = {}) {
  const { db, sendJson, readRequestBody, requireSession, resolveScope, canPermission } = deps;
  if (!db) throw new Error('mountLicensingRoutes requires db');
  ...
  function routeError(res, error) { write(res, error.statusCode || 400, envelope(null, error.message || 'Licensing/Tenancy request failed', { code: error.code || 'LICENSING_ERROR' })); }
  ...
  function handle(req, res, url) {
    ...
    try {
      if (rest.length === 2 && rest[0] === 'licensing' && rest[1] === 'install' && req.method === 'POST') {
        body(req).then(input => {
          write(res, 200, envelope(lic.installLicense(db, scope.companyId, input)));
        }).catch(error => routeError(res, error));
        return true;
      }
      
      if (rest.length === 2 && rest[0] === 'licensing' && rest[1] === 'license' && req.method === 'GET') {
        write(res, 200, envelope(lic.getLicense(db, scope.companyId)));
        return true;
      }
      
      if (rest.length === 2 && rest[0] === 'licensing' && rest[1] === 'seats' && req.method === 'GET') {
        write(res, 200, envelope(lic.checkSeatUsage(db, scope.companyId)));
        return true;
      }
      
      if (rest.length === 2 && rest[0] === 'tenants' && rest[1] === 'provision' && req.method === 'POST') {
        body(req).then(input => {
          write(res, 201, envelope(lic.provisionTrialTenant(db, input.tenant_id, input.tenant_name, input.edition)));
        }).catch(error => routeError(res, error));
        return true;
      }
...
module.exports = { mountLicensingRoutes };
