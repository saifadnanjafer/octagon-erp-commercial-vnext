// clean-room; behavior modeled on platform/server/views-fields.js (proprietary self, not copied)
'use strict';
...
// 'snapshot' (T1.5.1, R1.5): a relation field whose value is materialized
// (copied, not live-joined) from a referenced record's field at a named
// doc-state transition — see snapshot_of validation below and
// fields/snapshot-fields.js for the materialization hook.
const TYPES = new Set(['text', 'number', 'date', 'select', 'textarea', 'snapshot']);
const SNAPSHOT_STATE_RE = /^[A-Za-z][A-Za-z0-9_]{0,63}$/;
...
function normalOptions(value, type) {
  ...
  if (parsed.length > 200) throw Object.assign(new Error('عدد الحيارات كبير جداً'), { statusCode: 400 });
...
// snapshot_of config shape (T1.5.1, R1.5): declared on a custom field of
// type 'snapshot'. `entity`/`ref_field` describe the relation ON THIS
// entity's own record (ref_field holds the FK id pointing at `entity`);
// `field` is the field to copy FROM the referenced record; `at_transition_to`
// is the doc-state name (see state/doc-state.js transitions[].to) that
// triggers materialization. Documented in TASK.md.
function normalizeSnapshotOf(value) {
  const parsed = typeof value === 'string' ? safeJson(value, null) : value;
  if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed)) {
    throw Object.assign(new Error('snapshot_of مطلوب لحقول اللقطة (كائن JSON)'), { statusCode: 400 });
  }
  const entity = normalText(parsed.entity, 120);
  const field = normalText(parsed.field, 64);
  const refField = normalText(parsed.ref_field, 64);
  const atTransitionTo = normalText(parsed.at_transition_to, 64);
  if (!validEntity(entity)) {
    throw Object.assign(new Error('snapshot_of.entity غير صالح'), { statusCode: 400 });
  }
  if (!FIELD_KEY_RE.test(field)) {
    throw Object.assign(new Error('snapshot_of.field غير صالح'), { statusCode: 400 });
  }
  if (!FIELD_KEY_RE.test(refField)) {
    throw Object.assign(new Error('snapshot_of.ref_field غير صالح'), { statusCode: 400 });
  }
  if (!SNAPSHOT_STATE_RE.test(atTransitionTo)) {
    throw Object.assign(new Error('snapshot_of.at_transition_to غير صالح'), { statusCode: 400 });
  }
  return { entity, field, ref_field: refField, at_transition_to: atTransitionTo };
}
...
function rowToField(row) {
  return {
    ...
    snapshot_of: safeJson(row.snapshot_of, null),
  };
}
...
function createViewsFieldsHandler(deps) {
  ...
  function audit(current, action, entity, recordId, before, after) {
    try { writeAudit(db, { entity, recordId, user: current.id, action, before, after }); } catch (_) {}
  }
  ...
  async function dispatch(req, res, url) {
    ...
    try {
      ...
      if (group === '_custom-fields') {
        if (req.method === 'GET' && rest.length === 2) {
          const rows = db.prepare('SELECT entity, key, label_ar, type, options, position, snapshot_of FROM x_custom_fields WHERE entity = ? ORDER BY position, key').all(entity);
        ...
        if (req.method === 'POST' && rest.length === 2) {
          ...
          const snapshotOf = type === 'snapshot' ? normalizeSnapshotOf(body.snapshot_of) : null;
          const field = { entity, key, label_ar, type, options: normalOptions(body.options, type), position: Number.isFinite(Number(body.position)) ? Math.max(0, Math.floor(Number(body.position))) : 0, snapshot_of: snapshotOf };
          db.prepare('INSERT INTO x_custom_fields (entity, key, label_ar, type, options, position, snapshot_of) VALUES (?, ?, ?, ?, ?, ?, ?)').run(entity, key, label_ar, type, JSON.stringify(field.options), field.position, snapshotOf ? JSON.stringify(snapshotOf) : null);
        ...
        if ((req.method === 'PATCH' || req.method === 'DELETE') && rest.length === 3 && FIELD_KEY_RE.test(key)) {
          const current = requireAdmin(req, res); if (!current) return true;
          const row = db.prepare('SELECT entity, key, label_ar, type, options, position, snapshot_of FROM x_custom_fields WHERE entity = ? AND key = ?').get(entity, key);
          ...
          const after = {
            ...before,
            label_ar: body.label_ar == null ? before.label_ar : normalText(body.label_ar, 120),
            type: body.type == null ? before.type : normalText(body.type, 20),
            options: body.options == null ? before.options : normalOptions(body.options, body.type || before.type),
            position: body.position == null ? before.position : Math.max(0, Math.floor(Number(body.position))),
            // Note: editing the field DEFINITION never touches values already
            // materialized into records (x_records.data.custom[key]) — those
            // stay frozen regardless of what happens here (T1.5.1 invariant:
            // "snapshots never re-resolve"). This only changes config used for
            // FUTURE materializations.
            snapshot_of: body.snapshot_of === undefined ? before.snapshot_of : (body.snapshot_of === null ? null : normalizeSnapshotOf(body.snapshot_of)),
          };
          if (!after.label_ar || !TYPES.has(after.type) || !Number.isFinite(after.position)) { fail(res, 400, 'إعداد الحقل غير صالح'); return true; }
          if (after.type === 'snapshot' && !after.snapshot_of) { fail(res, 400, 'snapshot_of مطلوب لحقول اللقطة (snapshot)'); return true; }
          if (after.type !== 'snapshot') after.snapshot_of = null;
          db.prepare('UPDATE x_custom_fields SET label_ar = ?, type = ?, options = ?, position = ?, snapshot_of = ? WHERE entity = ? AND key = ?').run(after.label_ar, after.type, JSON.stringify(after.options), after.position, after.snapshot_of ? JSON.stringify(after.snapshot_of) : null, entity, key);
...
module.exports = { createViewsFieldsHandler, _internal: { normalizeViewConfig, normalOptions, normalizeSnapshotOf, rowToField } };
