// ported; relocated Octagon-owned code originally from platform/client/ui-crud.js
/**
...
(function () {
  ...
  CrudInstance.prototype.openDetail = function (id, scrollToChatter) {
    ...
    api('/' + this.entity + '/read/' + encodeURIComponent(id)).then(function (env) {
      ...
      var historyId = 'history-' + self.entity + '-' + rec.id;
      var body =
        ...
        '</div>' +
        '<div class="oxc-chatter-wrap"><h4 class="oxc-chatter-title">🕘 سجل التغييرات (Audit History)</h4>' +
          '<div id="' + esc(historyId) + '" class="oxc-chatter-host"></div>' +
        '</div>';
      ...
      var historyEl = self.root.querySelector('#' + CSS.escape(historyId));
      if (window.OX.historyPanel && typeof window.OX.historyPanel.mount === 'function') {
        try { window.OX.historyPanel.mount(historyEl, self.entity, rec.id); } catch (e) { toast('تعذر تحميل سجل التغييرات', 'error'); }
      } else if (historyEl) {
        historyEl.innerHTML = '<div class="oxc-muted oxc-chatter-placeholder">وحدة سجل التغييرات غير محمّلة بعد (OX.historyPanel)</div>';
      }
